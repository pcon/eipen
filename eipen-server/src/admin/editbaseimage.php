<?php
	...
	if (isset($_GET['id'])) {
		...
		if ($imageid < 0) {
			setMessage("Invalid base image id", "bad");
		...
		if (isset($_POST['submit'])) {
			...
			if (ereg('^[A-Za-z0-9\(\)\. -]{1,}$', $name)) {    //check name
				if (ereg('^[A-Za-z0-9_\./-]{1,}$',$image)) {	   //check image name
					...
					$query = "SELECT imageid FROM admin_base_images WHERE imageid=$imageid";
					...
					mysql_query("UPDATE admin_base_images SET name=\"$name\", image=\"$image\", fullvirt=$fullvirt, ".
						"typeid=$typeid WHERE imageid=$imageid") 
			...
			$query = "SELECT name, image, fullvirt, typeid FROM admin_base_images WHERE imageid=$imageid";
			$result = mysql_query($query);
...
?>
<html>
<head><title>Eipen - Edit Base Image</title>
