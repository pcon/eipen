<?php
	...
	if (isset($_GET['image']) && strcmp($type, 'usage') == 0) {
		...
		$query = "SELECT date, times FROM stats_image_dates WHERE image = \"$image\" LIMIT 7";
		...
		$query = "SELECT date, SUM(times) AS times FROM stats_image_dates GROUP BY date ORDER BY date DESC LIMIT 7";
		...
		$query = "SELECT dayName, times FROM stats_image_byday";
		...
		$query = "SELECT hour, times FROM stats_image_byhour";
